...
const PDV = {
  ...
  async writePropose({ title='', category='', background='', billId='' }={}) {
    ...
    return _sendToPDV({
      ...
      where: { svc_url: 'https://democracy.hondi.net', label: '안건 게시판' },
  ...
  async writeAgree({ billId='', billTitle='', voteWeight=1 }={}) {
    ...
    return _sendToPDV({
      ...
      when:  { generated_at: now, period_start: now, period_end: now },
      where: { svc_url: 'https://democracy.hondi.net', label: '동의 확보 게시판' },
  ...
  async writeVote({ billId='', billTitle='', side='', voteWeight=1 }={}) {
    ...
    return _sendToPDV({
      ...
      when:  { generated_at: now, period_start: now, period_end: now },
      where: { svc_url: 'https://democracy.hondi.net', label: '하원 의결' },
  ...
  async writeVeto({ billId='', billTitle='', reason='' }={}) {
    ...
    return _sendToPDV({
      ...
      when:  { generated_at: now, period_start: now, period_end: now },
      where: { svc_url: 'https://democracy.hondi.net', label: '상원 검토' },
  ...
  async writeResult({ billId='', billTitle='', result='', forPct=0, againstPct=0 }={}) {
    ...
    return _sendToPDV({
      ...
      when:  { generated_at: now, period_start: now, period_end: now },
      where: { svc_url: 'https://democracy.hondi.net', label: '의결 결과' },
  ...
  async writeConsult({ userMsg='', aiMsg='' }={}) {
    ...
    return _sendToPDV({
      ...
      who:   { ipv6, role: 'citizen', recipients: ['gopang-pdv'] },
      when:  { generated_at: now, period_start: now, period_end: now },
      where: { svc_url: 'https://democracy.hondi.net', label: 'AI 민주주의 상담' },
